...
export default function Reservation() {
  ...
  // Edit mode state (for updating existing reservation)
  ...
  useEffect(() => {
    async function loadRestaurants() {
      try {
        ...
        // Check if restaurantId is in URL params and pre-populate
        const restaurantIdFromUrl = searchParams.get("restaurantId");
        if (restaurantIdFromUrl && restaurantsList.length > 0) {
          const restaurant = restaurantsList.find((r) => r.id === restaurantIdFromUrl);
          if (restaurant) {
  ...
  // Load existing reservation if reservationId param is present
  useEffect(() => {
    const reservationId = searchParams.get("reservationId");
    const selectServer = searchParams.get("selectServer") === "true";
    
    if (reservationId && currentUser && restaurants.length > 0) {
      async function loadExistingReservation() {
        try {
          // Find which restaurant this reservation belongs to
          const allReservations = await getCurrentDinerReservationsFromLedger(currentUser.uid);
          ...
          if (reservation) {
            ...
            setFormData((prev) => ({
              ...
              serverId: reservation.metadata?.serverId || null,
              serverName: reservation.metadata?.serverName || null,
              preferences: reservation.metadata?.preferences || [],
              birthdayName: reservation.metadata?.birthdayName || "",
              anniversaryYears: reservation.metadata?.anniversaryYears || "",
              specialRequests: reservation.metadata?.specialRequests || "",
            }));
            
            // Load servers if schedule is published
            if (selectServer && restaurant) {
              setTimeout(() => {
  ...
  }, [searchParams, currentUser, restaurants]);
  ...
  // Handle server selection update for existing reservation
  const handleUpdateServer = async () => {
    if (!editingReservation || !formData.serverId || !formData.serverName) {
      setUpdateError("Please select a server");
      return;
    }
    ...
    try {
      ...
      // Update editing reservation state
      setEditingReservation({
        ...editingReservation,
        metadata: {
          ...editingReservation.metadata,
          serverId: formData.serverId,
          serverName: formData.serverName,
        },
      });
      ...
      setUpdateError("Failed to update server selection. Please try again.");
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    // If editing reservation and just updating server, don't create new reservation
    if (editingReservation && searchParams.get("selectServer") === "true") {
        ...
        <section className="reservation-section">
          ...
          <form onSubmit={handleSubmit} className="reservation-form">
            ...
            {loadingServers ? (
              ...
              <div className="form-group">
                <label>
                  {editingReservation && searchParams.get("selectServer") === "true"
                    ? "Select Your Server *"
                ...
                {editingReservation && searchParams.get("selectServer") === "true" && (
                  <div className="update-notice" style={{
                    ...
                    📅 The schedule for your reservation date is now available! Select your preferred server below.
                  </div>
                )}
                ...
                {editingReservation && searchParams.get("selectServer") === "true" && (
                  <div style={{ marginTop: "16px" }}>
                    <button
                      ...
                      disabled={isUpdatingServer || !formData.serverId}
                      className="update-server-btn"
                      style={{
                        background: formData.serverId ? "#4da3ff" : "#666",
                        color: "white",
                        border: "none",
                        padding: "12px 24px",
                        borderRadius: "6px",
                        fontSize: "16px",
                        fontWeight: "600",
                        cursor: formData.serverId ? "pointer" : "not-allowed",
                        transition: "background 0.2s",
                      }}
                    >
                      {isUpdatingServer ? "Updating..." : "Update Server Selection"}
                    </button>
                    {updateSuccess && (
                      <div style={{
                        marginTop: "12px",
                        ...
                        ✅ Server selection updated successfully!
                      </div>
                    )}
                    {updateError && (
                      <div style={{
